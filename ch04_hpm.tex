...
\newcite{roller:2016:emnlp}, and all work in this chapter constitutes
...
for features inputted to a classifier: \newcite{weeds:2014:coling} proposed
Diff, which uses {\em only} the vector difference, while
\newcite{roller:2014:coling} noted the importance of the additional
...
\newcite{roller:2014:coling} find vector differences to be useful and outperform
a concatenation based model (Concat), \newcite{weeds:2014:coling} find the opposite:
a simple, linear model based on vector concatenation outperforms the vector
difference model in their experiments. Later,
\newcite{vylomova:2016:acl} examined the models further and found that vector
differences is robust at predicting a variety of
lexical relationships, both with and without lexical overlap, but
\newcite{shwartz:2016:acl} report better results using simple concatenation.

\newcite{levy:2015:naacl} focus on the issue of lexical
...
and therefore will predict that pairs like (\lit{animal}, \lit{cactus}) will
be predicted as hypernymy, because \lit{animal} looks like a typical hypernym,
and \lit{cactus} looks like a typical hyponym.
...
prototypicality concerns. \newcite{levy:2015:naacl} proposed a custom SVM
kernel {\em Ksim}, which explicitly includes the critical inner product
missing in linear models. \newcite{kruszewski:2015:tacl} focus more on the
...
features. Both \newcite{levy:2015:naacl} and \newcite{kruszewski:2015:tacl}
...
of \newcite{levy:2015:naacl}, as they contain analytical terms necessary for
...
For our final classifier, we use a regularized SVM with an RBF-kernel, though decision
...
Indeed, some other works have already found some evidence that generalizing to
non-hypernymy relations is possible with existing models from the literature.
\newcite{vylomova:2016:acl} finds that a Diff model \cite{weeds:2004:coling}
is able to successfully distinguish between many lexical relations,
including hypernymy, meronymy, gender (\lit{king/queen}), object-location
(\lit{fish/aquarium}), and action-object (\lit{zip/coat}). They also find that
...
Dagan~(\shortcite{shwartz:2016:cogalex2,shwartz:2016:cogalex1}) apply the
...
[tobacco]} and \lit{[cat] vaguely like a [dog]}. The K\&H meronymy H-features
identifies clear possessive constructions (\lit{stallion's [tail]}) and mirror
the meronymy patterns proposed by \newcite{girju:2006:cl}, though
...
\section{Other Works in Distributional Relationship Detection}
