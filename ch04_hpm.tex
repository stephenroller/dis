...
data sets (including BLESS and DBPedia), and use only {\em related} words
...
To compare our H-feature model to theirs, we use their published data sets,
...
non-noun lexical items. Since data sets are not explicitly annotated with
...
We evaluate our multi-relation H-feature detector on several existing data
sets covering a wide variety of relations.

The first data set we use is {\bf BLESS} \cite{baroni:2011:gems}, which was also used
...
The second data set we use is {\bf EVAL} \cite{santus:2015:ws}, which contains
...
The third data set we use is {\bf Root9} \cite{santus:2016:lrec} contains
...
The final data set we use is {\bf K\&H+N} \cite{necsulescu:2015:starsem}, our
...
data set to be higher quality and more interesting than other data sets, but
