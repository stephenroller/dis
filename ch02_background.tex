...
In the current political environment, one can imagine RTE systems may become
an important tool against so-called ``fake news.''
...
Presently, there exist many approaches of RTE systems. One broad class, based
heavily on deep learning, tends to focus on learning entire representations
from scratch
...
perform incredibly well statistically, they can sometimes be later outperformed
by simpler models which only focus on simple alignment
\cite{parikh:2016:emnlp}, or fail to learn basic properties about lexical
meaning \cite{pavlick:2016:acl}, and therefore can fail unpredictably,
...
\end{tikzpicture}
\end{minipage}
\caption{(left) Example contexts of the word \lit{dog}, and (right) cartoon drawing
...
\cite{hofman:1999:sigir,blei:2003:jmlr} and soft-plus ($\log(1 + x)$)
...
\cite{deerwester:1990:jsis,landauer:1997:pr} or Non-Negative Matrix
Factorization (NNMF) \cite{lee:2000:nips}. In dimensionality reduction, the
co-occurrence matrix $\mM$ is assumed to be factorizable into two
lower-rank matrices,
\begin{equation}
  {\bf M} = \mV\mC\trans
  \label{eqn:factorization}
\end{equation}
where $\mV$ is some lower dimension representation of word vectors, and $\mC$ is
...
Value Decomposition (SVD). In this formulation, we find a factorization
\begin{equation*}
...
\begin{align*}
  \mV & = \mA{\bf \Sigma}^{\lambda},\\
  \mC & = \mB{\bf \Sigma}^{(1 - \lambda)},
\end{align*}
where $\lambda$ is some real number on the interval [0, 1]; traditionally,
the value $\lambda = 1$ is used, placing all the weight of the singular values
onto the word matrix \cite{deerwester:1990:jsis}; however, some more recent
works have found this parameter best if it's adjusted \cite{needcite}.

The SVD is particularly special, because it is uniquely defined for any
complex-valued matrix, has a direct connection to the eigendecomposition, and
provides us with a natural interpretation. One especially useful property is
...
induct or identify all its possible sense meanings \cite{needcite}. It is clear
