...
cosine similarity}:
\begin{equation}
  \text{cos}(\vu, \vv) = \frac{\sum_i u_iv_i}{\sqrt{\sum_i u_i^2 \sum_i v_i^2}} = \frac{{\vu}^\top{\vv}}{\norm{\vu} \norm{\vv}}
  \label{eq:cos}
\end{equation}
...
contexts for the word {\em chased} would be {\ctx nsubj+dog} and {\ctx dobj+tail}. Distributional Spaces defined in this manner tend to emphasize the
