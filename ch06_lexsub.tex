...
\section*{Copy paste from paper}

Lexical substitution~\cite{mccarthy:2007:semeval} is a task in which word meaning in context is described not through dictionary senses but through substitutes (paraphrases) chosen by annotators. % Given a target word in context, the annotator generates one or more substitutes for the target in the given context. As multiple annotators label the same target, the result is a weighted list of substitutes, where the weight indicates how many annotators chose a particular substitute. 
...
\section{Conclusion}
We have presented \ourmeasparam, a simple new measure for assessing the appropriateness of
a substitute in a particular context for the Lexical Substitution
task. The measure assesses the fit of the substitute both to the
target word and the sentence context. It significantly outperforms
comparable baselines from prior work, and does not require any additional
lexical resources. An analysis indicates its
performance improvements derive primarily from a tendency to lean more strongly
on unigram statistics than baselines. In future work, our measure could be simplified
by implementing the bias as a single scaling parameter.
