...
is performed for all examples in our data set, and then the features$(H, w)$
...
relations, and we train a binary classifier. The second data set is {\bf
...
which just guesses randomly for the balanced LEDS data set, and always
...
\begin{table}
  ...
  \begin{tabular}{|lc|c|}
    \hline
    {\bf Classifier} & {\bf Space} & {\bf Acc.}\\
    \hline\hline
    \multicolumn{3}{|c|}{{\bf LEDS}}\\
    \hline
    Random Baseline           &     -   & .50\\
    \newcite{baroni:2011:gems}   & Window 2 & .81\\
    \newcite{roller:2014:coling} & Window 2 & {\bf .85}\\
    \newcite{baroni:2011:gems}   & Sentence & .78\\
    \newcite{roller:2014:coling} & Sentence & .82\\
    \hline\hline
    \multicolumn{3}{|c|}{{\bf BLESS}}\\
    \hline
    Always guess {\em no-relation} & - & .46\\
    \newcite{baroni:2011:gems}   & Window 2 & .76\\
    \newcite{roller:2014:coling} & Window 2 & {\bf .84}\\
    \newcite{baroni:2011:gems}   & Sentence & .73\\
    \newcite{roller:2014:coling} & Sentence & .80\\
    \hline
  \end{tabular}
...
see further evidence of this in Section~\ref{sec:multiprototype}.
...
important in Section~\ref{sec:lexmem}.

We also test our interpretation of Asym as measuring a form of {\em Selective}
Distributional Inclusion. After training the model's parameters from the entire
BLESS data set, we compare the model's learned hyperplane to the {\em context
...
dimensions to highly compliment existing similarity measures based on
the Distributional Inclusion Hypothesis. A careful reader should note this
is necessary, but {\em not} sufficient to show the model obeys the SDIH, as
we are effectively comparing unsupervised models with supervised ones.

We measure every word pair's similarity using three similarity measures:
our baseline, cosine similarity, should not change substantially between the
original and selective distributional spaces; The second similarity measure,
ClarkeDE, measures roughly what percentage of the hyponyms' mass is contained
within the hypernym, i.e.,
\begin{equation*}
  {\text ClarkeDE}(H, w) = \frac{\sum_i min(H_i, w_i)}{\sum_i H_i};
\end{equation}
ClarkeDE similarity

We Finally, we
measured the ability of three distributional similarity measures
