...
To rememdy this, we proposed Asym, a supervised classifier based on the
{\em vector difference} between two word vectors. The asym model provided
rough conditional probability that two words $a$ and $b$ exhibited a lexical
relation:
\begin{equation*}
  \text{asym({\bf \hat H}, {\bf \hat e})} = \sigma\left({\bf w}^\top ({\bf H} - {\bf e})\oplus({\bf \hat H} - {\bf e})^2\right) + b
\end{equation*}
