...
logically follows from a given text NEEDCITE.
...
to become common NEEDCITE. Yet these systems ultimately must work over
individual lexical items to form a conclusion, and require knowledge about
the relationships between lexical items. Consider the following example:
...
``You shall know a word by the company it keeps'' \cite{firth:1957:la}. In Distributional
Semantics, representations of word meaning are automatically induced by
counting or modeling the {\em contexts} in which a word appears Distributional
Semantics is often sometimes called Vector Space Models of language, as words
are represented as vectors a high-dimensional vector space.  Words with similar
semantics will have similar vectors in this induced geometric space. As these

In this work, we approach the question as to whether Distributional Semantics
can be leveraged to predict the sort of lexical inferences necessary in tasks
like Question Answering and Recognizing Textual Entailment. Namely, we present
techniques and models for predicting certain kinds of lexical relationships,
entailments, and subsitutions using Distributional Semantics. In Lexical
Relationship Detection, we try to specifically identify whether two words
exhibit specific lexical relationships, like hypernymy (is-a relationships) or
meronymy (has-a relationships). 

What is distributional semantics, why is it useful here
