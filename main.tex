...
target word in a sentential context \cite{roller:2016:naacl}. In this document,
we also argue that Lexical Substitution is a form of lexical entailment in
context.
...
typically computed in modern applications. Next, we briefly discuss the Recognizing
...
these tasks are completely sufficient, but one goal of our thesis to show
developments in these tasks improves practical RTE.
...
Understanding systems. One of the original papers detailing the
...
information about lexical items in a more automatic fashion. Next, we will
discuss Distributional Semantics and Vector Space Models, which provide an
automatic induction of word meaning using only large, readily available
corpora.
...
\newcite{mikolov:2013:iclr,pennington:2014:emnlp}), while syntactic spaces often
have a millions, e.g. \newcite{baroni:2010:cl}. Efficiently dealing with these
...
There has been a great deal of research around predicting lexical relationships
...
More recently, groups have begun researching how lexical relationships could be
mined automatically using distributional semantics. Since distributional
semantics provides a way of estimating word meaning automatically from only
large, unannotated corpora, it widely hoped that it can be used to identify
word relationships \cite{baroni:2011:gems,baroni:2012:eacl}. Ideally, this could
be used to augment existing lexical resources like WordNet, or for bootstrapping
a WordNet-like resource in new languages, especially ones where large corpora
exist, but not the resources to hire a team of lexicographers.

One lasting hypothesis specifically about hypernymy detection has been the
Distributional Inclusion Hypothesis (DIH) \cite{zhitomirsky-geffet:2005:acl},
which states that the contexts in which a hypernym appears should be a superset
of all its hyponyms, or subordinate words. Though this seems to be strictly
false\footnote{Consider {\em ``\#The animal barked loudly at its bowl of animal
food.''}}, a considerable amount of work has assumed it to be at least
partially true.
...
\cite{weeds:2004:coling,baroni:2011:gems}. Many of the proposed measures were
...
Increasingly, it became obvious that unsupervised measures did not work as well
as the community hoped, the community began working on entailment detection as
a supervised task. \newcite{baroni:2012:eacl} proposed, as a preliminary
baseline of a novel dataset, training a simple baseline classifier to predict
whether word pairs were either hypernyms or nonhypernyms. Although they
reported accuracies in the high 90s, others later realized this was due to
issues of {\em lexical memorization}, or a special kind of overfitting
...
Until now, we have assumed that words are mononymous, or that they have only
one meaning. However, words like ``bright'' have multiple meanings, which
change depending on context, as in ``bright girl'' and ``bright coat.'' This is
called {\em polysemy}, and is a major issue in Natural Language Understanding,
since it adds yet another layer of possible ambiguity in sentence meaning.
...
happens to a word based on its used in a sentential context. Since c. 2007,
one popular way to measure this has been the Lexical Substitution task.  In the
Lexical Substitution task, the program is provided with a sentential
context,\footnote{The CoInCo dataset also provides the sentences before and
after the target word, but these are usually ignored in the literature.} and
must provide {\em substitutes} which can replace the given target word, while
...
LexSub still rely on single-point estimates
...
this task using distributional semantics, currently unsupervised measures hold
a narrow lead \cite{melamud:2015:naacl,melamud:2016:conll}.
...
it is also an important proxy to improvements on Textual Entailment. Namely,
synonymy is highly sensitive to the effects of polysemy, as shown in our
ongoing example, and therefore is necessary to explicitly model. Furthermore,
it is possible to view Lexical Substitution as a kind of lexical entailment
{\em in context}: if the substitutes replace the target and preserve the
meaning of the sentence, then it follows that the target {\em entails} the
substitute. While other lexical relationships are also affected by
hypernymy\footnote{``Crane'' is both a ``bird'' and an ``equipment.''}

\section{Completed work}

In this section, we discuss our recent publications relativent to this
...
the Lexical Substitution task, and explain why it improves upon prior work.
...
\begin{table}
  ...
  \begin{tabular}{|lc|cc|}
    ...
    \hline
    \newcite{baroni:2012:eacl}   & Window 2 & .81          & .76      \\
    \newcite{roller:2014:coling} & Window 2 & {\bf .85}    & {\bf .84}\\
    \newcite{baroni:2012:eacl}   & Sentence & .78          & .73      \\
...
%However, we show
%
%\begin{figure} \centering \includegraphics[width=0.45\textwidth]{plots/window}
...
\newcite{levy:2015:naacl}. We also include two additional novel baselines,
Concat+Asym, which was used in \newcite{beltagy:2016:cl}. We cannot include a
