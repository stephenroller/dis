...
To answer this question, we consider how the problem could be recast as a
supervised classification problem, and propos Asym, a simple supervised model
that is inherent asymmetric \cite{roller:2014:coling}. At its core, the model
is inspired by the famous result of \newcite{mikolov:2013:iclr}, who observed
that vector subtraction can be used to perform some kinds of analogical
reasoning in some kinds of Distributional Spaces: e.g., ${\tt king} - {\tt man}
+ {\tt woman} \approx {\tt queen}$. Interestingly, this vector subtraction
approach reasonably models both grammatical relationships (singular/plural,
verb conjugations) and some semantic relationships (gender, capital/country).
