...
In 2014, we presented a novel, supervised model for the task of Hypernymy Detection.
...
property that they're symmetric: $\text{cosine}(a, b) = \text{cosine}(b, a)$.
However, lexical relationships are very often asymmetric: although {\em girl}
implies {\em child}, the the opposite surely does not hold. As such, trying to
perform lexical entailment based solely on symmetric measures will always
fail hard. This was recognized widely in the literature, and numerous
asymmetric, unsupervised similarity measures were proposed
\cite{weeds:2003:emnlp,zhitomirsky-geffet:2005:acl,clarke:2009:gems,kotlerman:2010:nle,santus:2013:thesis},
mostly inspired by the Distributional Inclusion Hypothesis.
However, their performance tended to be lackluster \cite{clarke:2009:gems} or
brittle \cite{kotlerman:2010:nle}. This lead to the question: are the measures
just sensitive to noise in distributional vectors, or is the Distributional
Inclusion Hypothesis fundamentally flawed?

To this end, we consider how the problem could be recast as a supervised
classification problem, and propos Asym, a simple supervised model that is
inherent asymmetric \cite{roller:2014:coling}. At its core, the model is
inspired by the famous result of \newcite{mikolov:2013:iclr}, who observed that
vector subtraction can be used to perform some kinds of analogical reasoning
in some kinds of Distributional Spaces: e.g.,
${\tt king} - {\tt man} + {\tt woman} \approx {\tt queen}$. Interestingly,
this vector subtraction approach reasonably models many sophisticated relationships
like gender, singular/plural, verb conjugations, and capital/country.


The most successful models tended to be based on the Distributional Inclusion
Hypothesis of \newcite{zhitomirsky-geffet:2005:acl}, which states that
the grammatical contexts of a hypernym should be a superset of its hyponyms'
contexts. However, most of these measures had lackluster or brittle
performance \cite{clarke:2009:gems,kotlerman:2010:nle}. Nonetheless, we
hypothesized these approaches were generally on the right track

One hypothesis we considered was the fundamental intuition behind the
Distributional Inclusion Hypothesis may still be correct

To remedy
 the task using supervised approaches. Prior to our work, only one other paper had tried
super
