...
\begin{equation*}
  \text{proj}_{\hat p}(x) = \left(\frac{x^T\hat p}{\|\hat p\|}\right)\hat p.
\end{equation*}
...
\begin{align*}
  ...
  & \qquad = \langle H_i^Tw, H_i^T\hat p_i, w_i^T\hat p_i, H_i^T\hat p_i - w_i^T\hat p_i\rangle
\end{align*}
...
\begin{tabular}{|lrrr|}
    ...
    Cosine binning only                & 71.5 & 78.7 & 77.7  \\
    \newcite{roller:2014:coling} only  & 70.1 & 77.3 & 77.2  \\
    \hline
...
\begin{align*}
  ...
  P(s | t) &= \frac{1}{Z_t}\exp\left\{s^T t\right\}\\ %{\sum_{s'}\exp\left\{s'\cdot t\right\}}
  P(s | C) &= \frac{1}{Z_C}\exp\left\{\sum_{c\in C}s^T\left[Wc + b\right]\right\}
\end{align*}
...
\begin{align*}
  ...
  P_n(s | C) &= \frac{1}{Z_n}\exp\left\{\sum_{c\in C}s^T c\right\} %{\sum_{s'}\exp\left\{\sum_{c\in C}{s'\cdot c}\right\}}
\end{align*}
...
magnitude of the dot product, $s^Tc$. This suggests one could even replace our
