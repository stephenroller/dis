...
explored a little bit in \newcite{snow:2004:nips}, who used syntactic Hearst
Patterns to make predictions about hypernymy, and then linearly interpolated
the probabilities of predictions in order to better conform to a prior. Later,
\newcite{snow:2006:acl} proposed a probabilistic model over {\em taxonomies},
based on the degree to which they violate the transitivity of hypernymy and
reflexiveness of co-hyponymy.

This
turns out to be an idea previously explored in the works of
\newcite{snow:2004:nips} and \newcite{snow:2006:acl}. 



For example, consider a system which makes the following predictions:

As discussed in the short term proposals, hypernymy prediction is not the only
important word relationship that could be predicted from distributional
information. Indeed, if we posit that the non-hypernymy relations can be
predicted from even moderate accuracy, then the additional information provided
could 

This is not the first time such a procedure has been proposed
