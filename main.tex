...
can benefit more from modeling Lexical Substitution as an entire task, rather
limiting this phenomenon to pure synonymy.
...
The in-context appropriateness, on the otherhand, attempts to directly model
...
given in Table~\ref{tab:cherry}. However, we note that the intutions explained
here are also backed up quantitatively in the original paper
\cite{roller:2016:naacl}. In our example, we see that all four measures tend
to pick excellent substitutes which {\em semantically} agree with the original
target.
