...
%this document we define it as any lexical relationship where a cooperative
%reader might accept 
%
%would logically assume a given consequent from a given
...
%has} eyes; a dog {\em has a} tail), but also many nonclassical ones. For
%example, 
%
%For
...
Lexical Entailment is a broad term for any number of ``semantic relations
that hold between lexical elements when the meaning of one element can be
inferred from the meaning of the other'' \cite{shnarch:2008:thesis}.
...
\newcite{kremer:2014:eacl} found that only 9\% of lexical substitutes were
