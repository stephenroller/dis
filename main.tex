...
end-to-end, complete RTE system developed by \newcite{needcite} to predict the
test portion of the RTE dataset. This ensures we are actually testing whether
Asym is able to contribute in a rich RTE pipeline.
...
a need for further study (c.f. Section~\ref{sec:multiprototype}).
...
predictions and errors made by each, and find that
Asym does substantially better at distinguishing hypernymy from co-hyponymy.
This is what we had hoped to find, given the findings in Section~\ref{sec:asym},
and that cosine is known to heavily favor co-hyponymy \cite{baroni:2011:gems}.
However, we also find that cosine features are better at discovering synonymy,
and that Asymmetric frequently mistakes antonymy as an entailing.

We also qualitatively compare the distributional similarity classifier with the classifier
