...
Since different types of distributional spaces exhibit different properties, we
evaluate our model on three different distributional spaces: the first two,
which use a simple Bag-of-Words context, differ in their context window size.
The {\em Window-2 BoW} space counts content words two words to the left and
right of targets as contexts, while the {\em Sentence BoW} space counts all
content words within complete sentence bounadries.
...
\begin{table}
  ...
  \hline
  Window 2 BoW     &    .76          & {\bf.84}       &     .81         & {\bf.85}         \\
  Sentence BoW     &    .73          &     .80            &     .78         &     .82            \\
  \typedmr         &     -           &     .82            &     .65         &  {\bf.85}            \\
% \mikolovr        &    .46          &     .77            \\
  \hline
  \end{tabular}
