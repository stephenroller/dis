\chapter{Hypernymy Detection and Asym}
\label{ch:lexmem}
...
Hearst patterns are \lit{X such as Y}; \lit{X for example Y}; and 
\lit{X including Y}. Each of these provides evidence that X is a hypernym of Y,
...
relationship between X and Z if provided \lit{X, such as Y and Z}. Some work
...
\newcite{lenci:2012:starsem} introduce the \invCL measure, which uses \Clarke to
...
into 17 general Categeories, like \lit{vehicle}, \lit{ground mammal} and
...
As we turn our attention to supervised methods (which will remain our focus for
...
pairs like (\lit{cactus}, \lit{animal}) may be classified as hypernymy, simply
because \lit{animal} nearly always a hypernymy, and \lit{cactus} is nearly
...
space of \newcite{baroni:2011:gems}, though this space was computed with
...
\begin{table}
  ...
  \caption{Accuracy of \newcite{baroni:2012:eacl} and Asym on BLESS and LEDS
    ...
    \newcite{baroni:2012:eacl} did not converge on the BLESS data set with
    TypeDM vectors.}
...
\newcite{baroni:2012:eacl} in all five direct comparisons, indicating our
